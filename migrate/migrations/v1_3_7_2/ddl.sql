ALTER TABLE `space_work_item_comment`
    ADD COLUMN `emojis` json NULL AFTER `reply_comment_id`;

ALTER TABLE `space_config`
    ADD COLUMN `comment_deletable_when_archived` int NULL COMMENT '任务归档时评论是否可删除;0:否,1:是' AFTER `comment_deletable`,
    ADD COLUMN `comment_show_pos` int NULL COMMENT '评论展示位置;0:底部,1:单独tab' AFTER `comment_deletable_when_archived`;

ALTER TABLE `space_work_item_v2`
    ADD COLUMN `reason` json NULL COMMENT '原因' AFTER `version_id`;
