...
func main() {
	...
	var dbHost = flag.String("host", "127.0.0.1", "db host")
	var dbPort = flag.Int("port", 3306, "db port")
	var dbUser = flag.String("user", "root", "user name")
	var dbPwd = flag.String("password", "123456", "user password")
	var dbName = flag.String("db", "test", "database name")
	var tpl = flag.String("template", "cmd/mysql-to-proto/template/proto.go.tpl", "template path")
	var file = flag.String("output", ".output/mysql-to-proto/sso.proto", "output file")

	flag.Parse()

	// dbName := "cs"
	connectionString := fmt.Sprintf("%s:%s@tcp(%s:%d)/%s?charset=utf8mb4&parseTime=true", *dbUser, *dbPwd, *dbHost, *dbPort, *dbName)
	fmt.Println(connectionString)

	db, err := Connect("mysql", connectionString)
	//Table names to be excluded
	exclude := map[string]int{"user_log": 1}
	if err != nil {
		fmt.Println(err)
	}
	if IsFile(*file) && !DelFile(*file) {
		fmt.Fprintf(os.Stderr, "del file:%s fail \n", file)
		return
	}
	// if IsFile(file) {
...
func (table *Table) Generate(filepath, tpl string) error {
...
func DelFile(f string) bool {
	err := os.Remove(f)
	if err != nil {
		fmt.Fprintf(os.Stderr, "DelFile:%s", err.Error())
		return false
	}
	return true
}
