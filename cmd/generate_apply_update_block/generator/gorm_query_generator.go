...
type GormQueryGenerator struct {
}

func NewGormQueryGenerator() *GormQueryGenerator {
	return &GormQueryGenerator{}
}

func (g *GormQueryGenerator) Generate(params *parser2.TemplateParams, fileName string) error {
	...
	for _, msg := range originalMessages {
		...
		if !strings.Contains(msg.Comments, "[db]") {
			continue
		}
