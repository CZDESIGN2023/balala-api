...
import (
	"fmt"
	"go-cs/cmd/dart-proto-generator/generator"
...
const (
	...
	OutputDartTableSqlFilePath    = ".output/generated/dart/pb/bean/types_sql.dart"
	OutputDartFieldEventFilePath  = ".output/generated/dart/pb/bean/types_field_event.dart"
...
func main() {
	...
	//---------------------------------------------
	fmt.Println("开始生成dart表sql语句文件...")
	err = generator.GenerateDartTableSqlFile(tmplParams, OutputDartTableSqlFilePath)
	if err != nil {
		fmt.Printf("生成dart表sql语句文件失败：%v\n", err)
		return
	}
	fmt.Printf("dart表sql语句文件生成完成，已保存至：%s。\n", OutputDartTableSqlFilePath)
