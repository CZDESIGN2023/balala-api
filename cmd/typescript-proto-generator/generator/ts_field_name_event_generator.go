...
const fieldNameEventDartTemplate = `// Auto-generated Typescript code. DO NOT MODIFY.
// Source: {{.InputFilePath}}

{{ $field_type := "int" }} {{ $field_defaultvalue := "0" }}{{range .Messages}}{{ $messageName := .Name }}
class {{.Name}}ProtoField {
  {{range .Fields}}public static {{ToHump .Name}}:String = "{{.Name}}";
  {{end}}
}

class {{.Name}}Event {
  {{range .Fields}}public static  {{ToHump .Name}}:String = "c.{{.Name}}";
  {{end}}
}
{{end}}
`
